...
public class RenderKatana extends RenderItemBase {
	...
	public RenderKatana(JsonElement jsonElement) {
		...
		if (jsonElement.getAsJsonObject().has("sheath")) {
			this.sheathStack = new ItemStack(Objects.requireNonNull(ForgeRegistries.ITEMS.getValue(new ResourceLocation(jsonElement.getAsJsonObject().get("sheath").getAsString()))));
		} else {
